...
const projects: Project[] = [
  {
    ...
    italic: "Hybrid RAG",
    desc: "Production hybrid RAG backend letting insurance adjusters query 10+ years of aviation claims data in plain English. LangGraph routes every query down an analytical (SQL) or semantic (pgvector) path and returns a grounded answer plus a downloadable CSV.",
    ...
    diagram: [
      { x: 12, y: 18, label: "adjuster · NL query", cls: "" },
      { x: 50, y: 18, label: "router · classify + filter", cls: "hi" },
      { x: 14, y: 48, label: "SQL node · validate", cls: "bl" },
      { x: 86, y: 48, label: "vector node · pgvector", cls: "bl" },
      { x: 50, y: 72, label: "insight + CSV", cls: "cr" },
      { x: 50, y: 92, label: "answer · S3 download", cls: "dk" },
    ],
  ...
  },
  {
    ...
    title: "NeuroScan",
    italic: "AI",
    desc: "Production-ready medical web app assisting radiologists with rapid brain-tumor triage. Classifies 2D MRI scans into four categories — glioma, meningioma, pituitary, or healthy — running real-time inference on commodity CPU cloud, no GPU required.",
    ...
    diagram: [
      { x: 12, y: 18, label: "MRI upload · PWA", cls: "" },
      { x: 50, y: 18, label: "FastAPI · async", cls: "bl" },
      { x: 88, y: 18, label: "JWT + Argon2", cls: "" },
      { x: 88, y: 50, label: "Cloudinary · blobs", cls: "cr" },
      { x: 50, y: 50, label: "EfficientNet-B0", cls: "hi" },
      { x: 12, y: 50, label: "Postgres · SQLModel", cls: "" },
  ...
  },
  {
    ...
    diagram: [
      { x: 10, y: 18, label: "300k lead DB", cls: "cr" },
      { x: 42, y: 18, label: "Synthflow · outbound", cls: "bl" },
      { x: 78, y: 18, label: "GPT-4o · intent", cls: "hi" },
      { x: 78, y: 48, label: "Decision Node", cls: "" },
      { x: 50, y: 72, label: "Qualified / Retry / DQ / DNC", cls: "" },
      { x: 14, y: 48, label: "fail-safe · 4o-mini", cls: "" },
      { x: 50, y: 92, label: "GHL · audit + narrative", cls: "dk" },
    ],
  ...
  },
  {
    ...
    desc: "Autonomous Python backend that turns a Master Marketing Checklist into hundreds of bespoke, client-specific SOPs on demand. One API call → cloned Drive tree, scraped references, GPT-4 Turbo generation, stylized .docx, linked back into the source sheet.",
    ...
    diagram: [
      { x: 10, y: 18, label: "REST trigger · biz profile", cls: "" },
      { x: 45, y: 18, label: "OAuth · clone sheet", cls: "bl" },
      { x: 82, y: 18, label: "Playwright · scrape refs", cls: "hi" },
      { x: 82, y: 50, label: "GPT-4 Turbo · 6-part SOP", cls: "" },
      { x: 45, y: 50, label: "python-docx", cls: "cr" },
      { x: 10, y: 50, label: "Drive tree · upload", cls: "" },
      { x: 45, y: 82, label: "write link → source cell", cls: "dk" },
    ],
  ...
  },
  {
    ...
    title: "PropAuto",
    italic: "CRM",
    desc: "Full-stack real-estate CRM and marketing platform. Investors scout distressed properties by geography, enrich owner contact data on credits, and run two-way SMS campaigns — with smart attribution that figures out which historic campaign an inbound reply actually belongs to.",
    ...
    diagram: [
      { x: 12, y: 18, label: "React · scout UI", cls: "" },
      { x: 50, y: 18, label: "FastAPI · enrich", cls: "bl" },
      { x: 88, y: 18, label: "Postgres · JSONB", cls: "cr" },
      { x: 88, y: 50, label: "Twilio · outbound", cls: "hi" },
      { x: 50, y: 50, label: "signed webhook in", cls: "" },
      { x: 12, y: 50, label: "sticky attribution", cls: "" },
