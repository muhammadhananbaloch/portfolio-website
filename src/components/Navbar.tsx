      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute top-16 left-0 w-full bg-background/95 backdrop-blur-xl border-b border-white/10 p-6 flex flex-col gap-6 md:hidden shadow-2xl"
          >
            {navLinks.map((link) => (
              <a
                key={link.href}
            ...
            <a 
              href="#contact" 
              onClick={() => setOpen(false)} 
              className="w-full text-center px-5 py-3 bg-foreground text-background rounded-full font-medium hover:opacity-90 transition-opacity"
            >
              Contact Me
            </a>
          </motion.div>
